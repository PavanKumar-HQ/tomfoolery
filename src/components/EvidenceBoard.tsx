  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto py-4 font-sans">
      ...
      <div className="border-3 border-[#1C1917] bg-white p-6 sm:p-8 neo-shadow-lg flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center space-x-3 shrink-0 font-mono text-xs font-bold">
          {canUnlockMore ? (
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              onClick={onUnlockNext}
              className="px-5 py-3 bg-[#F7A859] text-[#1C1917] border-2 border-[#1C1917] neo-shadow-sm neo-shadow-hover hover:bg-[#F8829C] transition-all flex items-center space-x-2"
            >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              onClick={onGoToGuess}
              className="px-5 py-3 bg-[#52A5EC] text-white border-2 border-[#1C1917] neo-shadow-sm neo-shadow-hover hover:bg-[#0C2340] transition-all flex items-center space-x-2"
            >
      ...
      {/* Grid of Evidence Cards with Stagger Animations */}
        ...
        {evidenceList.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className={`border-3 border-[#1C1917] transition-all relative p-6 flex flex-col justify-between ${
                isUnlocked
                  ? "bg-white neo-shadow-hover neo-shadow cursor-pointer hover:border-[#0C2340]"
                  : "bg-[#FAF8F5] opacity-60 neo-shadow-sm select-none border-dashed"
              }`}
            >
              ...
              <div>
                ...
                {/* Snippet / Preview */}
                <div className="font-sans text-sm min-h-[60px] flex items-center">
                  {isUnlocked ? (
                    <div className="space-y-2 w-full">
                      ...
                      {item.type === "waveform" && (
                        <div className="text-xs font-mono font-bold text-[#0C2340] bg-[#F7A859]/30 p-2 border border-[#1C1917] flex items-center justify-between">
                          <span>[AUDIO DISCOURSE WAVEFORM]</span>
                          <span className="text-[#1C1917]">{item.duration}</span>
                        </div>
                      )}
                      {item.type === "logo" && (
                        <div className="text-xs font-mono font-bold text-[#0C2340] tracking-wider uppercase bg-[#B3B5E6]/40 px-3 py-1.5 border border-[#1C1917] inline-block">
                          {item.logoText}
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="flex items-center space-x-2 text-xs font-mono text-[#78716C] font-semibold">
              ...
              {/* Bottom Action Bar */}
              <div className="mt-6 pt-4 border-t-2 border-[#1C1917] flex items-center justify-between font-mono text-xs font-bold">
                {isUnlocked ? (
                  <>
                    <span className="text-[#0C2340] flex items-center space-x-1.5 bg-[#7BB4EC]/30 px-2 py-0.5 border border-[#1C1917]">
                      <CheckCircle2 className="w-3.5 h-3.5 text-[#52A5EC]" />
                      <span>VERIFIED ✓</span>
                    </span>
                    <button 
                      type="button"
                      ...
                      className="px-3 py-1.5 bg-[#1C1917] text-white border-2 border-[#1C1917] neo-shadow-sm neo-shadow-hover hover:bg-[#0C2340] hover:text-[#F7A859] transition-all flex items-center space-x-1.5 cursor-pointer uppercase tracking-wider font-mono text-xs font-bold"
                    >
                      <span>INSPECT CLUE</span>
                      <Eye className="w-3.5 h-3.5 text-[#F7A859]" />
                    </button>
                  </>
                ) : (
